  ...
  config = flatten([
     for instance, params in local.raw_settings.instances : [
       for machine, p in params.machines : {
         ...
         boot_disk_type      = try(p.boot_disk_type, try(params.boot_disk_type, "gp2"))
         boot_disk_size      = try(p.boot_disk_size, params.boot_disk_size)
         extended_disk_type  = try(p.extended_disk_type, try(params.extended_disk_type, "gp2"))
         ...
         ebs_optimized       = try(p.ebs_optimized, try(params.ebs_optimized, false))
         ebs_encrypted       = try(p.ebs_encrypted, try(params.ebs_encrypted, true))
       }
     ]
  ])
